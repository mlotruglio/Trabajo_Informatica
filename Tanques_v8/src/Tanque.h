...
private:
	...
	float vida_max;
	float danio;
	float velocidad_disparo;
public:
	Tanque(vector3D _posicion, float _velocidad_max, float _orientacion, float _radiohitbox = 1);
	Tanque(vector3D _posicion, float _velocidad_max = 0, float _orientacion = 0, float _radiohitbox = 1, float _vida_max = 100, float _tiempo_recarga = 1, float _danio = 1, float _velocidad_disparo = 10);
	Tanque();
	...
	void setVelocidadDisparo(float);
	float getVelocidadDisparo();
