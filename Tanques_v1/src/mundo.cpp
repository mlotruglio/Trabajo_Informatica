...
void mundo::raton() {
		tanque.setturretorientation(camara.getorientation() + atan2(400.0 - estadosentradas.getmousepos()[0], 300.0 - estadosentradas.getmousepos()[1]) * 180.0 / 3.1415926535) ;
		static bool lastclik[4];
		
		if (estadosentradas.getmousestates()[0] && !lastclik[0]) {
...
void mundo::tecla()
{
	...
	if (estadosentradas.getkeystates()['a'] || estadosentradas.getkeystates()['A'])
		tanque.setbodyorientationrel(3.0f);
...
void mundo::loadmap(const char* filename) {
	...
	if (file == NULL) {
		...
		while (!feof(file)) {
			...
			fscanf(file, "%c", &key);
			if (key == '^') {
				tanque.setposition((float)-j * escala, 0.1, (float)i * escala);
